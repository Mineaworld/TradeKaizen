...
export default function CalendarPage() {
  ...
  const getDayProfit = (day: number) => {
    ...
    return mockTradeData[dateStr]?.profit || null
  }

  const monthlyTotal = Object.values(mockTradeData).reduce((sum, day) => sum + day.profit, 0)
        ...
        <DialogTrigger asChild>
          <div
            className={`
              ...
              ${profit > 0 ? 'bg-green-50 dark:bg-green-950/30' : ''}
              ${profit < 0 ? 'bg-red-50 dark:bg-red-950/30' : ''}
            `}
          >
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-7 gap-4">
        {Array.from({ length: firstDayOfMonth }).map((_, index) => (
          <div key={`empty-start-${index}`} className="min-h-[100px] p-2 border rounded-lg bg-muted/20" />
        ))}
        ...
        {Array.from({ length: 6 - lastDayOfMonth }).map((_, index) => (
          <div key={`empty-end-${index}`} className="min-h-[100px] p-2 border rounded-lg bg-muted/20" />
        ))}
      </div>
        ...
        <CardContent>
          <div className="flex justify-between items-center">
            <div>
              ...
              <div className={`text-2xl font-bold ${monthlyTotal >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <div className="text-right">
              ...
              <div className="text-2xl font-bold">{Object.keys(mockTradeData).length}</div>
            </div>
          </div>
        </CardContent>
      </Card>
